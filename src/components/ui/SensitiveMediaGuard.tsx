    ...
    <div className={`relative group/sensitive overflow-hidden ${className}`}>
      ...
      {!showContent && (
        <div className={`absolute inset-0 z-20 flex flex-col items-center justify-center bg-black/60 backdrop-blur-md transition-all duration-300 ${isTiny ? 'px-1 py-2' : 'px-2 py-3'}`}>
          <div 
            className={`flex flex-col items-center flex-1 justify-center ${isTiny ? 'gap-1' : 'gap-3'} text-center w-full`}
            onClick={isXs ? handleReveal : undefined}
          >
            {isXs ? (
              <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center border border-white/20 shadow-lg">
                <AlertTriangle className="text-yellow-400/90" size={16} />
              </div>
            ) : (
              <>
                <div className={`${isSm ? 'w-8 h-8' : 'w-12 h-12'} rounded-full bg-white/5 flex items-center justify-center border border-white/10 shadow-lg mb-1`}>
                  <AlertTriangle className="text-yellow-400/80" size={isSm ? 16 : 24} />
                </div>
                ...
                <button
                  ...
                  className={`flex items-center justify-center gap-1 bg-white text-neutral-900 rounded-full font-extrabold transition-all active:scale-95 shadow-md
                    ${isSm ? 'px-3 py-1 text-[10px]' : 'mt-4 px-5 py-2 text-xs hover:bg-neutral-100'}
                  `}
                >
                  <Eye size={isSm ? 11 : 14} />

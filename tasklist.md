...
- [x] Quyết định approach xử lý block trong existing conversations
- [x] Implement block enforcement cho message creation
...
- [x] Quyết định có notify message reaction hay không: Đã chốt **KHÔNG** gửi thông báo (kể cả 1-1 hay nhóm) để tránh làm phiền người dùng.
- [x] Đánh dấu hoàn thành (không cần code thêm).
...
- [x] Tạo `onFriendRequestDeleted` CF để cleanup notification khi hủy lời mời
- [x] Sửa xử lý từ chối kết bạn thành DeleteDoc
...
**Đề xuất:** Xóa logic tự động gán ở BE, bắt buộc client truyền `newAdminId` khi `Creator` rời nhóm. Nếu nhóm chỉ còn 1 người, tự động gọi hàm `disbandGroup`.

- [x] Cập nhật quy trình nghiệp vụ UC-NT11: bỏ fallback, bắt buộc truyền admin mới, xử lý giải tán nhóm 1 người.
- [x] Cập nhật logic `leaveGroup` trong `groupService.ts` và UI hook.
...
- [x] Quyết định approach: đổi thành Delete DB khi reject để kích hoạt cleanup notification
...
- [x] Cập nhật spec: lời mời kết bạn được theo dõi realtime (không cần refresh)
...
- [x] Sửa `searchUsers` CF: lọc bỏ user có quan hệ block (cả hai chiều) khỏi kết quả tìm kiếm
...
| MISSING-02 | Block phải ngăn message trong existing conversation           | 🟡 MISSING | [x]    |
| MISSING-03 | Notification khi react message                                | 🟡 MISSING | [x]    |
| MISSING-04 | Cleanup notification khi hủy/từ chối lời mời kết bạn      | 🟡 MISSING | [x]    |
| MISSING-05 | `searchUsers` CF không lọc user bị chặn khỏi kết quả tìm kiếm | 🟡 MISSING | [x]    |
| DRIFT-01   | Sửa lỗi rời nhóm: bắt buộc truyền admin mới, tự giải tán nhóm | 🟠 DRIFT   | [x]    |
| DRIFT-02   | Reject friend request: xóa document thay vì update            | 🟠 DRIFT   | [x]    |
| DRIFT-03   | Spec: ghi nhận realtime subscription cho friend request       | 🟠 DRIFT   | [x]    |
| SPEC-01    | Bổ sung giới hạn ghi âm + cập nhật flow                       | 🔵 SPEC    | [x]    |
| SPEC-02    | Bổ sung bước Image Cropper vào flow đổi avatar/cover          | 🔵 SPEC    | [ ]    |
| SPEC-03    | Điều chỉnh search Admin: tên hoặc email vs. chỉ email         | 🔵 SPEC    | [x]    |

...
**1.2. Sub-collection `friends` (Bên trong `users/{uid}`)**

- `friendId` (String): Document ID.
- `createdAt` (Timestamp).

**1.3. Sub-collection `blockedUsers` (Bên trong `users/{uid}`)**
...
**1.4. Sub-collection `feeds` (Bên trong `users/{uid}` - Bảng tin Fan-out)**
